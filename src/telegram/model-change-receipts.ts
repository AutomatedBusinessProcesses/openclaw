...
export function formatModelChangeReceiptNotice(receipts: ModelChangeReceipt[]): string {
  ...
  const fallbackText = latest.fallbackRefs?.length
    ? `\nFallback: ${latest.fallbackRefs.join(", ")}`
    : "";
  if (receipts.length === 1) {
    return [
      `OpenClaw model changed: ${previousDisplayName(latest)} -> ${displayName(latest)}`,
      `Live route: ${latest.ref}${fallbackText}`,
      "Control-plane receipt, generated before the model runs.",
    ].join("\n");
  }
  ...
  return [
    `OpenClaw model changed ${receipts.length} times since the last Telegram receipt.`,
    `Live route: ${latest.ref}${fallbackText}`,
    ...hiddenLine,
    ...changeLines,
    "Control-plane receipt, generated before the model runs.",
